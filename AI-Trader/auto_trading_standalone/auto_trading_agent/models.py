...
SUPPORTED_EXCHANGES = {"paper"}
SUPPORTED_NETWORKS = {"paper"}
...
class TradingRequest(BaseModel):
    ...
    exchange_network: Optional[str] = Field(
        default="paper",
        description="Exchange network (paper only)",
    )
    ...
    def validate_exchange(cls, value):
        ...
        if lowered not in SUPPORTED_EXCHANGES:
            raise ValueError(f"Unsupported exchange '{value}'. Only 'paper' is supported.")
        return lowered
    ...
    def validate_exchange_network(cls, value):
        ...
        if lowered not in SUPPORTED_NETWORKS:
            raise ValueError("Only 'paper' exchange network is supported.")
        return lowered
